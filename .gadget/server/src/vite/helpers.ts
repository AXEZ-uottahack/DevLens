...
export const isRemixOrReactRouterFrameworkType = (type: FrontendType): type is FrontendType.Remix | FrontendType.ReactRouterFramework => {
  return type === FrontendType.Remix || type === FrontendType.ReactRouterFramework;
};
...
export const getFrontendTypeByPluginsUsed = (config: Record<string, any>): FrontendType => {
  ...
  const hasReactRouterPlugin = config.plugins?.some((pluginOptions: any) => {
  ...
  if (hasReactRouterPlugin) {
    type = FrontendType.ReactRouterFramework;
  }
