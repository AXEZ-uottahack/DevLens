...
function buildComputedView(client, operation) {
  const f = operation.variables ? async (variables = {}, options) => {
    let resultVariables;
    ...
    return await (0, import_computedViews.computedViewRunner)(
      client.connection,
  ...
  return Object.assign(f, operation, {
    plan: function(operationName, variables, namespace) {
      return (0, import_computedViews.computedViewOperation)(operationName, operation.defaultSelection, variables, void 0, namespace);
    }
  });
}
function buildModelComputedView(operation) {
  const f = operation.variables ? async function(variables = {}, options) {
    let resultVariables;
    if (operation.variables) {
      resultVariables = {};
      for (const [name, variable] of Object.entries(operation.variables)) {
