...
export function buildComputedView<VariablesT, ResultT>(
  ...
  const f = operation.variables
    ? async (variables: Record<string, any> = {}, options?: { select?: FieldSelection }) => {
        ...
        return await computedViewRunner(
          client.connection,
          operation.gqlFieldName,
    ...
    : async (options?: { select?: FieldSelection }) => {
        return await computedViewRunner(
          client.connection,
          operation.gqlFieldName,
          operation.defaultSelection,
  ...
  return Object.assign(f, operation, {
    plan: function (operationName: string, variables: VariablesOptions, namespace: string) {
      return computedViewOperation(operationName, operation.defaultSelection, variables, undefined, namespace);
    },
  }) as any;
}
...
export function buildModelComputedView<VariablesT, ResultT>(
  ...
  const f = operation.variables
    ? async function (this: AnyModelManager, variables: Record<string, any> = {}, options?: { select?: FieldSelection }) {
        let resultVariables: VariablesOptions | undefined;
        if (operation.variables) {
          resultVariables = {};
          for (const [name, variable] of Object.entries(operation.variables)) {
    ...
    : async function (this: AnyModelManager, options?: { select?: FieldSelection }) {
        return await computedViewRunner(
          this.connection,
          operation.gqlFieldName,
          operation.defaultSelection,
